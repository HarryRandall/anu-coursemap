...
function failure(message: string, roleKey: string): AdminRoleActionResult {
  return { ok: false, roleKey, message };
}
...
export async function setAdminUserRole(
  ...
  if (!uuidPattern.test(userId) || !roleKeyPattern.test(roleKey)) {
    return failure("That user or role is not valid.", roleKey);
  }

  const { viewer, canAccessAdmin } = await getAuthContext();
  if (!viewer || !canAccessAdmin) {
    return failure("Administrator access is required.", roleKey);
  }
  ...
  try {
    ...
    if (error) {
      ...
      if (message.includes("at least one administrator")) {
        return failure(
          "Coursemap must keep at least one administrator.",
          "admin",
        );
      }
      if (message.includes("does not exist")) {
        return failure("That user or role no longer exists.", roleKey);
      }
      return failure("Coursemap could not update that role.", roleKey);
    }
...
export async function setAdminRolePermission(
  ...
  try {
    ...
    if (error) {
      const message = error.message.toLowerCase();
      if (message.includes("required for the admin role")) {
        return {
          ok: false,
          enabled: true,
      ...
      if (message.includes("only be granted by the admin role")) {
        return {
          ok: false,
          enabled: false,
          message: "Only the Admin role can grant administrator access.",
        };
      }
      if (message.includes("does not exist")) {
        return {
