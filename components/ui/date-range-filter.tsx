    ...
    >
      <PopoverTrigger asChild>
        <Button
          ...
          className={cn("h-9", selected?.from && "text-foreground")}
          size="md"
          variant="secondary"
        >
      ...
      <PopoverContent align="end" className="w-auto p-0">
        <div className="flex flex-col sm:flex-row">
          <Calendar
            ...
            selected={selected}
          />
          <div className="flex shrink-0 flex-col gap-0.5 border-t border-border p-2 sm:border-t-0 sm:border-l">
            {presets.map((preset) => (
              <button
                className="cursor-pointer rounded-md px-3 py-1.5 text-left text-sm font-medium text-foreground/80 transition-colors hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
                key={preset.days}
            ...
            {selected?.from ? (
              <button
                className="mt-1 cursor-pointer rounded-md border-t border-border/60 px-3 py-1.5 text-left text-sm font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
                onClick={() => {
                  if (state) {
                    setDraft(undefined);
