...
export function CoursePicker({
  ...
  const filtered = useMemo(
    () =>
      courses.filter((course) => {
        if ((courseCounts.get(course.code) ?? 0) >= courseOccurrenceLimit(course.code)) {
          return false;
        }
  ...
  return (
    <Modal onClose={onClose} labelledBy="course-picker-title" className="h-[min(46rem,calc(100dvh-3rem))] w-full max-w-4xl">
      <div className="flex items-start justify-between gap-4 border-b border-zinc-100 px-5 py-4">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-400">
            Add to {term.name} {term.year < 2029 ? term.year : ""}
          </p>
          <h2 id="course-picker-title" className="mt-0.5 text-xl font-bold tracking-tight text-zinc-900">
            Find a course
          </h2>
        </div>
      ...
      <div className="flex items-center gap-3 border-b border-zinc-100 px-5">
        ...
        />
        <span className="shrink-0 text-xs text-zinc-400">{filtered.length} courses</span>
      </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 sm:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="overflow-y-auto p-2" role="listbox" aria-label="Course results">
          {filtered.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-1 p-8 text-center">
              <Search size={22} className="text-zinc-300" />
              <p className="mt-2 text-sm font-medium text-zinc-700">No courses match those filters</p>
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-zinc-900">{course.name}</span>
                  ...
                  <ArrowRight size={16} className="shrink-0 text-zinc-300" aria-hidden="true" />
                </button>
              );
            })
          )}
        ...
        <aside className="hidden overflow-y-auto border-l border-zinc-100 bg-zinc-50/70 p-5 sm:block">
          {selected ? (
            <>
              <CourseToken code={selected.code} accent={selected.accent} size="lg" />
              <p className="mt-4 text-[11px] font-bold uppercase tracking-wider text-zinc-400">
                {selected.code}
              </p>
              <h3 className="mt-1 text-lg font-bold leading-tight tracking-tight text-zinc-900">
                {selected.name}
              </h3>
              ...
              <Button variant="primary" fullWidth onClick={() => choose(selected)}>
                <Plus size={16} /> Add to {selected ? term.shortName : ""}
              </Button>
              <ButtonLink variant="subtle" fullWidth href={`/courses/${selected.code}`} className="mt-2">
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2 text-center">
              ...
              <p className="text-sm font-medium text-zinc-700">Select a course</p>
