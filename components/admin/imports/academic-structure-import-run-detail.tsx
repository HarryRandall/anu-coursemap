...
import type { AcademicStructureImportRunDetail } from "@/lib/coursemap/admin-academic-structure-imports";
...
function statusTone(status: string): Tone {
  ...
  if (["needs_review", "unchanged", "pending"].includes(status)) {
    return "warning";
  }
  if (["accepted", "succeeded", "valid"].includes(status)) return "success";
  return "neutral";
}
...
export function AcademicStructureImportRunDetailView({
  detail,
}: {
  detail: AcademicStructureImportRunDetail;
}) {
  const { run } = detail;
    ...
    <AppShell admin currentBreadcrumbLabel={`Run #${run.runNumber}`}>
      ...
      <div className="mx-auto w-full max-w-7xl space-y-5 pb-10">
        <h1 className="sr-only">
          Academic structure import run {run.runNumber}
        </h1>
        ...
        <header className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm text-zinc-700">
            {run.academicYear} {readable(run.structureKind)} import
          </span>
        ...
        {active ? (
          <Alert tone="brand">
            ...
            />
            <AlertDescription>
              This page updates automatically. The background worker continues
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <StatTile label="Accepted as draft" value={run.acceptedCount} />
        ...
        <dl className="grid gap-x-6 gap-y-3 border-y border-zinc-200 py-4 text-xs sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div>
            <dt className="text-zinc-500">Parser</dt>
        ...
        <DataTableShell>
          <Table className="min-w-[1040px]">
            <TableCaption>
              Structures in import run {run.runNumber}
            </TableCaption>
            <TableHeader>
              ...
              {detail.targets.map((target) => (
                <TableRow key={target.id}>
                  <TableCell>
                    ...
                    {target.title ? (
                      <span className="mt-0.5 block max-w-72 truncate text-xs text-zinc-500">
                        {target.title}
                      </span>
                    ) : null}
                    {target.errorSummary ? (
                      <span className="mt-1 block max-w-72 truncate text-xs text-rose-700">
                        {target.errorSummary}
                      </span>
                    ) : null}
                  </TableCell>
                  ...
                  <TableCell className="text-right text-xs text-zinc-600 tabular-nums">
                    {target.extraction
                      ? usd(target.extraction.costUsd)
                      : "Not run"}
                  </TableCell>
                  <TableCell>
                    {target.extraction ? (
                      <Badge
                        tone={statusTone(target.extraction.validationStatus)}
                      >
                  ...
                  <TableCell className="text-right">
                    <Link
                      aria-label={`Open ${target.code} import details`}
                      ...
                      href={`/admin/imports/structures/runs/${run.id}/targets/${target.id}`}
                    >
