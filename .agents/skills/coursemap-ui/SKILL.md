...
Read `components/AGENTS.md`, the affected route and a comparable existing page.
...
3. Import pinned primitives from `@reui/ui/*` and extended components from `@reui/components/*`. `components/ui` holds Coursemap compositions, not a second primitive library.
4. Follow `components/reui/README.md` when adding or updating upstream components. Preserve retained source and licence; review dependencies and keep only what the product needs.
