-- The importer now records level-gated and total-unit conditions, so the
-- published course detail exposes each condition's course level bounds for
-- the student page to evaluate level_units and units_total rules.
