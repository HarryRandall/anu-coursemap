...
import { ArrowRight, BookOpen, GraduationCap, Search, Shapes, X } from "lucide-react";
...
const kpis = [
  { icon: <GraduationCap size={18} />, value: "82", label: "degree programmes" },
  { icon: <Shapes size={18} />, value: "434", label: "majors, minors and specialisations" },
  { icon: <BookOpen size={18} />, value: "2,146", label: "requirement buckets" },
];
...
export default function AdminProgrammesPage() {
  ...
  const filtered = useMemo(
    () =>
      structures.filter(
        (row) =>
          `${row.code} ${row.name}`.toLowerCase().includes(query.toLowerCase()) &&
  ...
  return (
    <AppShell title="Programmes" subtitle="Degrees and academic structures" admin>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {kpis.map((kpi) => (
          <Card key={kpi.label} className="flex items-center gap-3 p-4">
            ...
            <div>
              <p className="text-lg font-bold tracking-tight text-zinc-900">{kpi.value}</p>
        ...
        <div className="flex flex-col gap-3 border-b border-zinc-100 bg-zinc-50/70 px-4 py-3 sm:flex-row sm:items-end">
          ...
          <Field label="Type" className="w-44">
            <Select
              ...
              value={type}
              onChange={setType}
              options={["All structures", "Degree", "Major", "Minor", "Specialisation"].map((item) => ({ value: item, label: item }))}
            />
          </Field>
        </div>
          ...
          <table className="w-full min-w-[720px] text-left text-[12px]">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50/60 text-[10px] font-bold uppercase tracking-wider text-zinc-400">
                {["Structure", "Type", "Units", "Programmes", "Options", "State", "Changed", ""].map(
                  (head, index) => (
              ...
              {filtered.map((row) => (
                ...
                >
                  <td className="px-3 py-3 pl-4">
                    ...
                    <span className="block text-[11px] text-zinc-400">{row.name}</span>
                  </td>
                  ...
                  <td className="whitespace-nowrap px-3 py-3 text-zinc-500">{row.changed}</td>
                  <td className="px-3 py-3">
                    <button
                      type="button"
                      onClick={() => setSelected(row)}
      ...
      {selected && (
        <Drawer onClose={() => setSelected(null)} labelledBy="programme-detail-title" className="sm:w-[480px]">
          <header className="flex items-center justify-between gap-3 border-b border-zinc-100 px-5 py-3.5">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p id="programme-detail-title" className="truncate text-[13px] font-semibold text-zinc-900">
                  {selected.code}
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5">
            ...
            <p className="mt-1 text-xs text-zinc-500">{selected.code} · 2026 version</p>
            ...
            <div className="mt-4 grid grid-cols-3 divide-x divide-zinc-200 rounded-xl ring-1 ring-zinc-200">
              {[
                ...
                <div key={label} className="px-3 py-2.5">
                  ...
                  <p className="mt-0.5 text-[15px] font-bold tracking-tight text-zinc-900">{value}</p>
                </div>
              ))}
            </div>
            ...
            <section className="mt-6 border-t border-zinc-100 pt-5">
              <h3 className="text-[13px] font-semibold text-zinc-900">Requirement structure</h3>
              <div className="mt-3 flex flex-col gap-2.5">
                {[
                  ["Global constraints", `${selected.units} total units · catalogue-year locked`],
                  ["Compulsory courses", `${Math.max(4, Math.round(selected.options / 2))} parsed course requirements`],
                  ["Choice groups", `${Math.max(1, Math.round(selected.options / 3))} one-of or unit-based buckets`],
                  ["Elective rules", "Level, subject and programme eligibility retained"],
                ].map(([title, note]) => (
                  <div key={title} className="rounded-xl bg-zinc-50/70 p-3 ring-1 ring-zinc-200">
            ...
            <section className="mt-6 border-t border-zinc-100 pt-5">
              <h3 className="text-[13px] font-semibold text-zinc-900">Source snapshot</h3>
              <div className="mt-3 divide-y divide-zinc-100 rounded-xl ring-1 ring-zinc-200 text-[12px]">
                {[
          ...
          <footer className="flex items-center gap-2 border-t border-zinc-100 px-5 py-4">
            <Button variant="secondary" fullWidth onClick={() => notify(`${selected.code} queued for rule comparison`)}>
              Compare with 2025
            </Button>
            <Button variant="primary" fullWidth onClick={() => notify(`${selected.code} rules opened for edit (prototype)`)}>
              Edit rules
            </Button>
          </footer>
        </Drawer>
