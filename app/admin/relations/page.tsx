...
import { Check, GitBranch, Network, Search, TriangleAlert, X } from "lucide-react";
...
export default function AdminRelationsPage() {
  ...
  const filtered = useMemo(
    () =>
      relations.filter(
        (item) =>
          `${item.source} ${item.target} ${item.sourceText}`.toLowerCase().includes(query.toLowerCase()) &&
          (relationType === "All relations" || item.relation === relationType) &&
  ...
  return (
    <AppShell title="Relations" subtitle="Academic rule graph" admin>
      <div className="grid grid-cols-1 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-zinc-200/70 lg:grid-cols-[minmax(0,1fr)_20rem]">
        {/* Main */}
        <div className="min-w-0 border-b border-zinc-100 lg:border-b-0 lg:border-r">
          <div className="flex flex-col gap-3 border-b border-zinc-100 bg-zinc-50/70 px-4 py-3 sm:flex-row sm:items-end">
            <label className="flex h-10 flex-1 items-center gap-2 rounded-lg bg-white px-3 shadow-xs ring-1 ring-inset ring-zinc-200">
              ...
              />
              <span className="shrink-0 text-[11px] text-zinc-400">{filtered.length} edges</span>
            </label>
            <Field label="Relation" className="w-40">
              <Select
                ...
                options={[{ value: "All relations", label: "All relations" }, ...types.map((item) => ({ value: item, label: item }))]}
              />
            </Field>
              ...
              <table className="w-full min-w-[640px] text-left text-[12px]">
                <thead>
                  <tr className="border-b border-zinc-100 bg-zinc-50/60 text-[10px] font-bold uppercase tracking-wider text-zinc-400">
                    {["Source", "Relation", "Target", "Group", "Hardness", "Confidence", "State"].map(
                      (head, index) => (
                  ...
                  {filtered.map((relation) => (
                    <tr
                      ...
                      className={cn(
                        ...
                        selected?.id === relation.id ? "bg-brand-50/50" : "hover:bg-zinc-50/70",
                      )}
                    >
                      <td className="px-3 py-3 pl-4">
                        {isCourseCode(relation.source) ? (
                          ...
                          <span className="font-mono font-semibold text-zinc-900">{relation.source}</span>
                        )}
                      </td>
                      <td className="px-3 py-3">
                        <span className="inline-flex items-center gap-1.5 rounded-full bg-brand-50 px-2 py-1 text-[11px] font-medium text-brand-700 ring-1 ring-inset ring-brand-100">
                          <GitBranch size={12} />
                          {relation.relation}
                        </span>
                      </td>
                      <td className="px-3 py-3">
                        {isCourseCode(relation.target) ? (
                          ...
                          <span className="font-mono font-semibold text-zinc-900">{relation.target}</span>
                        )}
                      </td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3 text-zinc-600">{relation.hardness}</td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3">
                        <Badge tone={parseTone(relation.state)}>{relation.state}</Badge>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {selected ? (
            <>
              <header className="flex items-center gap-3 border-b border-zinc-100 px-4 py-3.5">
                ...
                >
                  {selected.state === "Review" ? <TriangleAlert size={17} /> : <Check size={17} />}
                </span>
                <div className="min-w-0">
                  <p className="text-[10px] uppercase tracking-wide text-zinc-400">Selected relation</p>
              ...
              <section className="border-b border-zinc-100 px-4 py-4">
                <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-400">Relation</p>
                <h2 className="mt-1 text-base font-bold tracking-tight text-zinc-900">
                  {selected.relation}
                </h2>
                <dl className="mt-3 divide-y divide-zinc-100 text-[12px]">
                  {[
                    ["Rule group", <code key="c" className="rounded bg-zinc-100 px-1.5 py-0.5 font-mono text-[10px]">{selected.group}</code>],
              ...
              <section className="border-b border-zinc-100 px-4 py-4">
                ...
                <p className="mt-2 text-[11px] leading-snug text-zinc-400">
                  The source span stays attached to the semantic tree and this derived edge.
                </p>
              </section>
              ...
              {selected.state === "Review" && (
                <footer className="flex flex-col gap-2 px-4 py-4">
                  <Button
                    variant="primary"
                    fullWidth
                    onClick={() => notify(`${selected.id} accepted for this prototype`)}
                  >
                    <Check size={15} /> Accept parsed relation
                  </Button>
                  <Button variant="secondary" fullWidth onClick={() => notify("Manual override draft opened")}>
                    Create override
                  </Button>
                </footer>
              )}
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2 p-8 text-center">
              ...
              <p className="text-sm font-medium text-zinc-600">Select a relation</p>
            </div>
          )}
        </aside>
      </div>
