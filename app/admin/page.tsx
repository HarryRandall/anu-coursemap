          ...
          <div className="grid divide-y divide-zinc-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-4">
            {kpis.map((kpi) => (
              <Link
                key={kpi.label}
                href={kpi.href}
                className="group flex min-h-20 items-center gap-3 p-4 transition hover:bg-zinc-50/80"
              >
                <span
                ...
                <ArrowRight
                  size={14}
                  className="shrink-0 text-zinc-300 transition group-hover:translate-x-0.5 group-hover:text-brand-600"
                />
              </Link>
            ))}
          </div>
