...
export default function RequirementsPage() {
  ...
  const metrics = [
    { key: "complete", label: "Completed", value: completed, dot: "bg-brand-500" },
    { key: "planned", label: "In your plan", value: planned, dot: "bg-brand-300" },
    { key: "remaining", label: "Still to plan", value: stillToPlan, dot: "bg-zinc-300" },
  ];
    ...
    <AppShell title="Requirements" subtitle={`${degree.code} · ${state.profile.catalogueYear} rules`}>
      ...
      <Card className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-[15px] font-semibold text-zinc-900">Degree progress</h2>
            <p className="mt-0.5 text-xs text-zinc-500">{degree.units}-unit programme</p>
          </div>
          <span className="text-sm font-semibold text-zinc-900">{completePercent}% complete</span>
        </div>
        ...
        >
          <span className="bg-brand-500" style={{ width: `${(completed / degree.units) * 100}%` }} />
          <span className="bg-brand-300" style={{ width: `${(planned / degree.units) * 100}%` }} />
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 divide-y divide-zinc-100 rounded-xl ring-1 ring-zinc-200 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          {metrics.map((metric) => (
            <div key={metric.key} className="flex items-center gap-2.5 px-4 py-3">
              <span className={cn("size-2 rounded-full", metric.dot)} />
              <div>
                <p className="text-sm font-semibold text-zinc-900">{metric.value} units</p>
      ...
      <Card className="mt-4 overflow-hidden">
        <div className="border-b border-zinc-100 px-5 py-4">
          <h2 className="text-[15px] font-semibold text-zinc-900">Degree requirements</h2>
        ...
        <div className="divide-y divide-zinc-100">
          {requirementGroups.map((group) => {
            const progress =
              progressByGroup[group.id] ?? { completed: 0, planned: 0, stillNeeded: group.total };
              ...
              <div key={group.id} className="flex items-center gap-4 px-5 py-4">
                <span
                  className={cn(
                    ...
                    done ? "bg-emerald-500 text-white ring-emerald-500" : "text-zinc-300 ring-zinc-300",
                  )}
                >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-semibold text-zinc-900">{group.name}</p>
                  ...
                  <div
                    ...
                    aria-hidden="true"
                  >
                    <span style={{ width: `${completedPercent}%`, background: group.colour }} />
                    <span
                      className="opacity-40"
                      style={{ width: `${plannedPercent}%`, background: group.colour }}
                    />
                  </div>
                ...
                <div className="hidden w-28 text-right sm:block">
                  ...
                  <p className="text-[11px] text-zinc-400">of {group.total} units</p>
                </div>
