...
import { ArrowRight, FilterX, Grid2X2, List, Plus, Search, UserRound } from "lucide-react";
    ...
    <AppShell title="Courses" subtitle={`${state.profile.catalogueYear} catalogue`}>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="flex items-center gap-3 border-b border-zinc-100 px-5">
          ...
          />
          <span className="shrink-0 text-xs text-zinc-400">{filtered.length} results</span>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:flex lg:items-end">
            <Field label="Subject" className="min-w-36">
              <Select
                ...
                value={subject}
                onChange={setSubject}
                options={[{ value: "All subjects", label: "All subjects" }, ...subjects.map((item) => ({ value: item, label: item }))]}
              />
            </Field>
            ...
            <Field label="Convener" className="min-w-40">
              <Select
                ...
                options={[{ value: "All conveners", label: "All conveners" }, ...conveners.map((item) => ({ value: item, label: item }))]}
              />
            </Field>
            <Button variant="ghost" size="sm" className="h-9 self-end" onClick={clear}>
              <FilterX size={15} /> Clear
            </Button>
          </div>

          <div className="flex self-end rounded-lg bg-white p-0.5 ring-1 ring-inset ring-zinc-200">
            {([
              ["list", List],
              ["grid", Grid2X2],
              ...
              <button
                ...
                className={cn(
                  ...
                  view === key ? "bg-zinc-100 text-zinc-900" : "text-zinc-400 hover:text-zinc-600",
                )}
              >
                <Icon size={16} />
              </button>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-1 px-5 py-16 text-center">
            ...
            <p className="text-xs text-zinc-400">Try removing one of your filters.</p>
            <Button variant="secondary" size="sm" className="mt-3" onClick={clear}>
              Clear all filters
            </Button>
          </div>
        ) : view === "list" ? (
          <div className="divide-y divide-zinc-100">
            {filtered.map((course) => (
              ...
              >
                <Link href={`/courses/${course.code}`} className="flex min-w-0 items-center gap-3">
                ...
                <div className="hidden min-w-0 items-center gap-2 md:flex">
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block truncate text-[11px] text-zinc-400">{course.school}</span>
                  </span>
                </div>

                <div className="hidden md:block">
                  <Badge tone={course.prerequisiteCodes.length ? "warning" : "success"}>
                ...
                <div className="flex shrink-0 gap-1.5">
                  <IconButton label={`Add ${course.code} to plan`} onClick={() => setPlanCourse(course)}>
                    <Plus size={16} />
                  </IconButton>
                  <Link
                    href={`/courses/${course.code}`}
                    aria-label={`View ${course.code}`}
                    className="grid size-9 place-items-center rounded-lg bg-white text-zinc-500 shadow-xs ring-1 ring-inset ring-zinc-200 transition hover:text-zinc-800 hover:ring-zinc-300"
                  >
                    <ArrowRight size={16} />
                  </Link>
                </div>
          ...
          <div className="grid grid-cols-1 gap-3 bg-zinc-50/60 p-4 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.map((course) => (
              ...
              >
                ...
                <p className="mt-4 text-[11px] font-bold uppercase tracking-wider text-zinc-400">
                  {course.code} · Level {course.level / 1000}
                </p>
                <h3 className="mt-1 text-base font-bold leading-tight tracking-tight text-zinc-900">
                  {course.name}
                </h3>
                ...
                <dl className="mt-4 space-y-0 divide-y divide-zinc-100 border-t border-zinc-100 text-[12px]">
                  <div className="grid grid-cols-[4.5rem_1fr] py-2">
                    ...
                    <dd className="text-zinc-700">{course.sessions.join(", ")}</dd>
                  </div>
                  <div className="grid grid-cols-[4.5rem_1fr] py-2">
                ...
                <div className="mt-auto flex gap-2 pt-4">
                  <Button variant="secondary" size="sm" onClick={() => setPlanCourse(course)}>
                    <Plus size={15} /> Add
                  </Button>
                  <ButtonLink variant="ghost" size="sm" href={`/courses/${course.code}`}>
